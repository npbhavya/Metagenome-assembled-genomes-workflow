...
for f in $reads
do 
	kraken2 --db $KRAKEN_DB --paired reads/"$f"_1.fastq reads/"$f"_2.fastq --threads 1 --report-zero-counts --use-names --report taxa/"$f"_kraken_report --output taxa/"$f"_kraken.out
done
